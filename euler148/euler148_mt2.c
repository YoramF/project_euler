 ...
 */
 ...
 static async_s_func_t async_s_func;
 ...
 */
 void exec_func (void *arg, void *res) {
    ...
    f_arg->r_val = sum;
    async_call(async_s_func, arg, res);
 }
  ...
  */
 void async_func (void *arg, void *res) {
    ...
    if (s_line < LASTLINE) {
        ...
        l_arg->l_line = s_line;
        async_launch(&exec_func, NULL, arg, arg, NULL, async_id);
    }
 }
...
void gen_values (int rows) {
 ...
 int main () {
    ...
    async_s_func = async_sync_func_init(&async_func, async_id);
    ...
    // lounch the first executions, callback/async func will keep launching new calls as long as we have not reached last line
    for (i = 0; i < EX_WORKERS; i++) {
         async_launch(&exec_func, NULL, &s_arg[i], &s_arg[i], NULL, async_id);       
    }
